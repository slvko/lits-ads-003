...
fs.readFile(inputFileName, 'utf8', (err, data) => {
  ...
  let contents = data.split('\n'),
    ...
    solutions = {
      prev: Array.apply(null, Array(h)).map(() => new BN(0)),
      curr: Array.apply(null, Array(h)).map(() => new BN(0))
    },
    jumps = new Map();
  ...
  for (let i = 0; i < h; i++) {
    solutions.prev[i] = new BN((w > 1) ? 1 : 0);
  ...
  for (let i = 1; i < w - 1; i++) {
    for (let j = 0; j < h; j++) {
      solutions.curr[j] = getSolutionForElement(j, i);
    }
    for (let j = 0; j < h; j++) {
      let char = matrix[j][i];
      if (jumps.has(char)) {
        jumps.set(char, jumps.get(char).add(solutions.curr[j]));
      }
    }
  ...
  function getSolutionForElement(hIndex, wIndex) {
    ...
    return solutions.prev[hIndex].add(jumps.get(char));
  }
